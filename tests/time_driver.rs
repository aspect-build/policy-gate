...
async fn timeout_wins_when_operation_and_deadline_become_ready_together() {
    let driver = TestTimeDriver::new();
    let deadline = driver.now() + Duration::from_secs(1);
    let result = tokio::spawn(async move {
        TestTimeDriver::timeout_at(deadline, TestTimeDriver::sleep_until(deadline)).await
    });
...
async fn dropping_sleep_removes_its_waker() {
    ...
    let mut cancelled = Box::pin(TestTimeDriver::sleep_until(
        driver.now() + Duration::from_secs(1),
    ));
    assert_eq!(
