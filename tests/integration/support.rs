...
use policy_gate::{
    ...
    PolicyGateLayer, PolicyGateLayerConfig, PolicyGateMetrics, RequestPolicy, TimeDriver,
...
impl DecisionSource<Uuid> for ScriptedDecisionSource {
    ...
    async fn get_subject_decision(
        ...
        let result = match action {
            GetAction::Return(result) => result.map(Some),
            GetAction::Gate { release, result } => {
                let admission = release.acquire().await.expect("gate must remain open");
                admission.forget();
                result.map(Some)
            }
            GetAction::Missing => Ok(None),
        };
...
pub(crate) struct ConfigOptions {
    ...
    pub(crate) decision_refresh_after: Duration,
}
...
impl Default for ConfigOptions {
    fn default() -> Self {
        Self {
            ...
            decision_refresh_after: Duration::MAX,
        }
    }
}
...
pub(crate) fn validated(options: &ConfigOptions) -> PolicyGateConfig {
    PolicyGateConfig::builder()
        ...
        .decision_refresh_after(options.decision_refresh_after)
        .build()
...
pub(crate) struct TestRuntime {
    ...
    pub(crate) watcher: policy_gate::DecisionWatcher,
...
pub(crate) fn runtime(source: Arc<ScriptedDecisionSource>, options: &ConfigOptions) -> TestRuntime {
    ...
    let (gate, watcher, health) = PolicyGate::new_with_metrics_and_time_driver(
        &validated(options),
        source,
        test_metrics(),
        time,
    );
    let layer = PolicyGateLayer::new_with_response(
        gate.clone(),
        &layer_config(),
        TestPolicy,
        AxumBodyAdapter,
    ...
    let task = tokio::spawn({
        ...
        async move {
            if let Some(delay) = delay {
                TestTimeDriver::sleep(delay).await;
            }
            tokio::select! {
                () = watcher => {}
                _ = shutdown_rx.recv() => {}
...
pub(crate) fn withdrawal(subject: &str) -> DecisionChange<Uuid> {
    DecisionChange {
        subject: subject.parse().expect("test subject must be a UUID"),
        decision: None,
    }
