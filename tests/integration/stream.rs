...
use crate::support::{
    ConfigOptions, GetAction, PAYMENT_URL, SUBJECT_A, ScriptedDecisionSource, TestSubject, change,
    response, start_runtime, start_streaming_call, start_streaming_call_at_path, wait_for_metric,
...
#[tokio::test]
async fn wrapped_response_latches_eof_and_removes_exact_length_metadata() {
...
#[tokio::test]
async fn denial_wakes_a_body_pending_on_its_inner_stream() {
    let source = Arc::new(ScriptedDecisionSource::default());
    let watch = source.push_live_watch(Vec::new()).await;
    let runtime = start_runtime(Arc::clone(&source), &ConfigOptions::default()).await;
    let call = start_streaming_call(&runtime.layer, SUBJECT_A).await;
    let response_probe = Arc::clone(&call.response_probe);
    let mut response_body = call.response_body;
    let response_task = tokio::spawn(async move { response_body.frame().await });

    while response_probe.polls.load(Ordering::Acquire) == 0 {
    ...
    let frame = tokio::time::timeout(Duration::from_secs(1), response_task)
        .await
        .expect("denial wakes the pending response body")
        .expect("body task joins")
        .expect("body emits denial trailers")
        .expect("denial trailers succeed");
    let status = Status::from_header_map(frame.trailers_ref().expect("trailers frame"))
        .expect("gRPC status trailers");
    assert_eq!(status.code(), Code::FailedPrecondition);
    runtime.stop().await;
}
...
#[tokio::test]
async fn pending_transition_observer_does_not_block_inner_body_wakes() {
    let source = Arc::new(ScriptedDecisionSource::default());
    let _watch = source.push_live_watch(Vec::new()).await;
    let runtime = start_runtime(Arc::clone(&source), &ConfigOptions::default()).await;
    let call = start_streaming_call(&runtime.layer, SUBJECT_A).await;
    let response_probe = Arc::clone(&call.response_probe);
    let response_sender = call.response_sender.clone();
    let mut response_body = call.response_body;
    let response_task = tokio::spawn(async move { response_body.frame().await });
    ...
    send_data(&response_sender, b"allowed");

    let frame = tokio::time::timeout(Duration::from_secs(1), response_task)
        .await
        .expect("inner body wakes the pending response body")
        .expect("body task joins")
        .expect("body emits data")
        .expect("data frame succeeds");
    assert_eq!(frame.into_data().expect("data frame"), "allowed");
    runtime.stop().await;
}

async fn assert_request_stream_is_reset_after_denial(path: &str) {
    let source = Arc::new(ScriptedDecisionSource::default());
    let watch = source.push_live_watch(Vec::new()).await;
    let runtime = start_runtime(Arc::clone(&source), &ConfigOptions::default()).await;
    let mut call = start_streaming_call_at_path(&runtime.layer, SUBJECT_A, path).await;
...
#[tokio::test]
async fn client_streaming_requests_are_reset_after_denial() {
    for path in [
        "/google.bytestream.ByteStream/Write",
        "/google.devtools.build.v1.PublishBuildEvent/PublishBuildToolEventStream",
    ] {
        assert_request_stream_is_reset_after_denial(path).await;
    }
}
