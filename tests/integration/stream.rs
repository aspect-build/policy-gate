...
async fn denial_wakes_a_body_pending_on_its_inner_stream() {
    ...
    let Poll::Ready(Some(Ok(frame))) = Pin::new(&mut call.response_body).poll_frame(&mut context)
    else {
        panic!("body must emit denial trailers after being woken");
    };
...
async fn pending_transition_observer_does_not_block_inner_body_wakes() {
    ...
    let wakes_before_data = wake_counter.count();
    send_data(&call.response_sender, b"allowed");
    assert!(wake_counter.count() > wakes_before_data);

    let Poll::Ready(Some(Ok(frame))) = Pin::new(&mut call.response_body).poll_frame(&mut context)
    else {
        panic!("body must emit data after being woken");
    };
