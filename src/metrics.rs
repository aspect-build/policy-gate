...
/// about.
///
/// Methods are called on the admission path, on every streaming frame, and from the watcher task.
/// They must not block, await, or panic. The events marked below as middleware events come from
/// the Tower layer and are never emitted unless the `tower-layer` feature is enabled.
pub trait PolicyGateMetrics: Copy + Send + Sync + 'static {
    ...
    fn unary_call(self) {}
    /// Records a failed or invalid decision-source unary call.
