    ...
    T: Subject + Display + FromStr,
{
    ...
    async fn get_subject_decision(&self, subject: &T) -> Result<Decision, DecisionSourceError> {
...
) -> Result<Decision, DecisionSourceError> {
...
fn decode_decision(state: i32) -> Result<Decision, DecisionSourceError> {
    match ProtoDecision::try_from(state) {
        Ok(ProtoDecision::Unspecified) => Ok(Decision::Unspecified),
        Ok(ProtoDecision::Allowed) => Ok(Decision::Allowed),
        Ok(ProtoDecision::Denied) => Ok(Decision::Denied),
